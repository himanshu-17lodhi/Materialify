...
export class IconPackManager {
  ...
  public async init(): Promise<void> {
    if (!this.getIconPackByName(MATERIAL_ICON_PACK_NAME)) {
      this.iconPacks.push(createMaterialIconPack(this.plugin));
    }
    ...
    for (let i = 0; i < loadedIconPacks.files.length; i++) {
      ...
      if (fileName.endsWith('.zip')) {
        ...
        if (iconPackName === MATERIAL_ICON_PACK_NAME) {
          continue;
        }
  ...
  public async loadAll(): Promise<void> {
    ...
    for (let i = 0; i < loadedIconPacks.folders.length; i++) {
      ...
      if (folderName === MATERIAL_ICON_PACK_NAME) {
        continue;
      }
    ...
    for (const zipFile in zipFiles) {
      if (zipFile === MATERIAL_ICON_PACK_NAME) {
        continue;
      }
