...
export default class IconizePlugin extends Plugin {
  ...
  async onload() {
    ...
    this.registerEvent(
      this.app.vault.on('create', (file) => {
        this.queueAutomaticIconRefresh(file.path);
      }),
    );
    ...
    this.registerEvent(
      this.app.vault.on('rename', (file, oldPath) => {
        ...
        IconCache.getInstance().invalidate(oldPath);
  ...
  private async removeSingleIcon(file: TFile): Promise<void> {
    ...
    if (!didUpdate) {
      ...
      if (automaticIconName) {
        materialIconTheme.refreshPath(this, file.path, file);
        this.refreshOpenedFileIcon(file.path);
        return;
      }
  ...
  private queueAutomaticIconRefresh(path: string): void {
    this.automaticIconRefreshQueue.add(path);

    if (this.automaticIconRefreshTimeout !== null) {
      window.clearTimeout(this.automaticIconRefreshTimeout);
    }

    this.automaticIconRefreshTimeout = window.setTimeout(() => {
      this.flushAutomaticIconRefreshQueue();
    }, 50);
  }

  private flushAutomaticIconRefreshQueue(): void {
    const paths = [...this.automaticIconRefreshQueue];
    this.automaticIconRefreshQueue.clear();
    this.automaticIconRefreshTimeout = null;

    const unresolvedPaths: string[] = [];
    for (const path of paths) {
      if (!this.refreshPathIcon(path)) {
    ...
    if (unresolvedPaths.length > 0) {
      window.setTimeout(() => {
        for (const path of unresolvedPaths) {
          this.refreshPathIcon(path);
        }
      }, 250);
    }
  }

  private refreshPathIcon(path: string): boolean {
    ...
    const refreshed = materialIconTheme.refreshPath(this, path, file);
    this.refreshOpenedFileIcon(path);
    this.notifyPlugins();
    return refreshed;
  }
  ...
  private getRenderableIcon(iconNameWithPrefix: string): string | undefined {
    ...
    if (!foundIcon) {
      ...
      foundIcon = getSvgFromLoadedIcon(
        this,
        iconNameWithPrefix.substring(0, iconNextIdentifier),
        iconNameWithPrefix.substring(iconNextIdentifier),
      );
    }

    if (!foundIcon && this.iconPackManager.getPreloadedIcons().length > 0) {
  ...
  private refreshOpenedFileIcon(path: string): void {
    const iconNameWithPrefix = icon.getByPath(this, path);
    ...
    for (const openedFile of getAllOpenedFiles(this)) {
      ...
      if (this.getSettings().iconInTabsEnabled) {
        if (iconNameWithPrefix) {
          const iconColor = this.getIconColor(path);
          void iconTabs.add(this, path, leaf.tabHeaderInnerIconEl, {
            iconColor,
          });
        } else {
          iconTabs.remove(leaf.tabHeaderInnerIconEl, {
      ...
      if (this.getSettings().iconInTitleEnabled) {
        ...
        if (!iconNameWithPrefix) {
          titleIcon.hide(activeView.inlineTitleEl);
          continue;
        }

        const renderableIcon = this.getRenderableIcon(iconNameWithPrefix);
        if (renderableIcon) {
          titleIcon.add(this, activeView.inlineTitleEl, renderableIcon, {
  ...
  async loadIconFolderData(): Promise<void> {
    ...
    if (!data) {
      ...
      this.data = data;
      // Ensure settings exists and has all default keys
